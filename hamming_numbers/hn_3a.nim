...
iterator func_hamming() : BigInt =
  ...
  proc merge(x, y: Mytype): Mytype =
    ...
    if xh < yh:
      ...
      let mlzy: Lazy[Mytype] = (val: nil, thnk: mthnk)
  ...
  proc u(s: Mytype, n: int32): Mytype =
    ...
    let mlzy: Lazy[Mytype] = (val: nil, thnk: mthnk)
    let frst = Mytype(hd: one, tl: mlzy)
